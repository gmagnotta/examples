 ...
 * 
 * see https://github.com/tensorflow/models/blob/v1.13.0/samples/languages/java/object_detection/src/main/java/DetectObjects.java
 */
public class DetectObjects {
  public static String detect(String modelLocation, String label, InputStream image) throws Exception {
    final String[] labels = loadLabels(label);
    try (SavedModelBundle model = SavedModelBundle.load(modelLocation, "serve")) {
      ...
      //for (int arg = 2; arg < args.length; arg++) {
        ...
        try (Tensor<Float> scoresT = outputs.get(0).expect(Float.class);
          ...
          System.out.printf("* Processing Image Stream\n");
          ...
          for (int i = 0; i < scores.length; ++i) {
            ...
            foundSomething = true;
            String str = String.format("Found %-20s (score: %.4f)\n", labels[(int) classes[i]], scores[i]);
            System.out.printf(str);
            result.append(str);
          }
          if (!foundSomething) {
            System.out.println("No objects detected with a high enough score.");
            result.append("No objects detected with a high enough score.");
          }
        }
  ...
  private static String[] loadLabels(String filename) throws Exception {
    InputStream inputStream = DetectObjects.class.getClassLoader().getResourceAsStream("labels/mscoco_label_map.pbtxt");
  ...
  private static Tensor<UInt8> makeImageTensor(InputStream image) throws IOException {
